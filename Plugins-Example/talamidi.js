...
const midsoune = async (m, {conn, text}) => {
  if (text.startsWith('https://')) {
    try {
      let res = await fetch(`https://midsouneapi-fee7b0be8faf.herokuapp.com/api/dtalamidi?url=${encodeURIComponent(text)}`);
    ...
    try {
      let res = await fetch(`https://midsouneapi-fee7b0be8faf.herokuapp.com/api/talamidi?drs=${encodeURIComponent(text)}`)
      let data = await res.json();
      const results = JSON.parse(data.result);
      let message = '';
